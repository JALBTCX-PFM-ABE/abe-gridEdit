...
if [ $SYS = "Linux" ]; then
    DEFS=NVLinux
    LIBRARIES="-L $PFM_LIB -lchrtr2 -lmisp -lpfm -lnvutility -lgdal -lxml2 -lpoppler -lGLU"
    export LD_LIBRARY_PATH=$PFM_LIB:$QTDIR/lib:$LD_LIBRARY_PATH
else
    DEFS="XML_LIBRARY WIN32 NVWIN3X"
    LIBRARIES="-L $PFM_LIB -lchrtr2 -lmisp -lpfm -lnvutility -lgdal -lxml2 -lpoppler -liconv -lwsock32"
...
if [ $SYS = "Linux" ]; then
    ...
    cp icons/$NAME.jpg $PFM_BIN/pfmIcons
else
    if [ ! $WINMAKE ]; then
        WINMAKE=release
    fi
