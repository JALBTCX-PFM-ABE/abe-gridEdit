  ...
  QDialog (parent, (Qt::WindowFlags) Qt::WA_DeleteOnClose)
{
  ...
  setWindowTitle ("gridEdit Overlays");
...
manageOverlays::slotBrowse (int id)
{
  ...
  if (!QDir (options->overlay_dir).exists ()) options->overlay_dir = options->input_chrtr2_dir;


  QFileDialog fd (this, tr ("gridEdit Overlay Input files"));
      ...
      if (!newfile.isEmpty())
        {
          ...
          strcpy (misc->overlays[id].filename, newfile.toLatin1 ());
