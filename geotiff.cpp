...
void geotiff (nvMap *map, MISC *misc, OPTIONS *options)
{
  ...
  min_x = misc->total_displayed_area.min_x + (misc->hatchr_start_x * misc->chrtr2_share->chrtr2_header[0].lon_grid_size_degrees);
  min_y = misc->total_displayed_area.min_y + (misc->hatchr_start_y * misc->chrtr2_share->chrtr2_header[0].lat_grid_size_degrees);
  max_x = misc->total_displayed_area.min_x + (misc->hatchr_end_x * misc->chrtr2_share->chrtr2_header[0].lon_grid_size_degrees);
  max_y = misc->total_displayed_area.min_y + (misc->hatchr_end_y * misc->chrtr2_share->chrtr2_header[0].lat_grid_size_degrees);
  ...
  if (misc->GeoTIFF_init || fabs (prev_mbr.min_x - min_x) > 0.00000001 ||
      ...
      fabs (prev_mbr.max_y - max_y) > 0.00000001)
    {
      ...
      if (poDataset != NULL)
              ...
              if (max_y >= GeoTIFF_slat && min_y <= GeoTIFF_nlat && max_x >= GeoTIFF_wlon && min_x <= GeoTIFF_elon)
                {
                  ...
                  if (full_res_image == NULL || full_res_image->width () == 0 || full_res_image->height () == 0)
                    {
                      QMessageBox::critical (0, gridEdit::tr ("gridEdit GeoTIFF import"), gridEdit::tr ("Unable to open or read GeoTIFF file!"));
                  ...
                  if (rasterCount < 3)
                    {
                      delete poDataset;
                      QMessageBox::critical (0, "gridEdit", gridEdit::tr ("Not enough raster bands in geoTIFF"));
                      return;
                    }
                  ...
                  for (int32_t i = 0 ; i < rasterCount ; i++)
                    {
                      ...
                      if (dataType[i] != "Byte")
                        {
                          delete poDataset;
                          QMessageBox::critical (0, "gridEdit", gridEdit::tr ("Cannot handle %1 data type").arg (dataType[i]));
                          return;
                        }
                  ...
                  if (full_res_image == NULL || full_res_image->width () == 0 || full_res_image->height () == 0)
                    {
                      QMessageBox::critical (0, "gridEdit", gridEdit::tr ("Unable to open image!"));
                      delete poDataset;
                      return;
                    }
        ...
        {
          QMessageBox::warning (map, gridEdit::tr ("gridEdit GeoTIFF"), gridEdit::tr ("Unable to read tiff file."));
        }
    }
  else
    {
