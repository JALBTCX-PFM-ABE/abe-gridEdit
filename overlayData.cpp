...
void overlayData (nvMap *map, MISC *misc, OPTIONS *options)
{
  ...
  double        tmp_x[2], tmp_y[2], minBounds[4], maxBounds[4];
  int32_t       lat_deg = 0, lat_min = 0, lon_deg = 0, lon_min = 0, file, ping, count, type, numShapes;
                      ...
                      if (strlen (string) > 2)
                        {
                          ...
                          if (tmp_y[0] >= misc->total_displayed_area.min_y && tmp_y[0] <= misc->total_displayed_area.max_y &&
                              tmp_x[0] >= misc->total_displayed_area.min_x && tmp_x[0] <= misc->total_displayed_area.max_x)
                            {
                              if (strlen (desc) > 2)
                                {
                                  sprintf (string, " %s", desc);
                                  map->drawText (QString (string), tmp_x[0], tmp_y[0], misc->overlays[i].color, NVFalse);
                                }
                          ...
                          if (shape->nVertices >= 2)
                            {
                              ...
                              map->fillPolygon (count, lon.data (), lat.data (), misc->overlays[i].color, NVTrue);
                            }
  ...
  //  Draw hand drawn contours if available

  if (misc->hd_contour_count)
    {
      //  Figure out the minimum trigger distance for starting and stopping line drawing.

      double x_trigger_dist = 999999999.0;
      double y_trigger_dist = 999999999.0;
      ...
      for (int32_t chrtr2 = 0 ; chrtr2 < misc->chrtr2_share->chrtr2_count ; chrtr2++)
        {
          if (misc->chrtr2_share->display_chrtr2[chrtr2] && !misc->chrtr2_share->lock_chrtr2[chrtr2])
            {
              x_trigger_dist = qMin (x_trigger_dist, misc->chrtr2_share->chrtr2_header[chrtr2].lon_grid_size_degrees);
              y_trigger_dist = qMin (y_trigger_dist, misc->chrtr2_share->chrtr2_header[chrtr2].lat_grid_size_degrees);
            }
        }
      x_trigger_dist *= 2.0;
      y_trigger_dist *= 2.0;
          ...
          for (int32_t j = 0 ; j < misc->hd_contour[i].count ; j++)
            {
              if (start)
                {
                  tmp_x[0] = misc->hd_contour[i].xyz[j].x;
                  tmp_y[0] = misc->hd_contour[i].xyz[j].y;
                  start = NVFalse;
                }
              else
                {
                  if (fabs (misc->hd_contour[i].xyz[j].x - tmp_x[0]) > x_trigger_dist || fabs (misc->hd_contour[i].xyz[j].y - tmp_y[0]) > y_trigger_dist)
                    {
                      start = NVTrue;
                    }
                  else
                    {
                      tmp_x[1] = misc->hd_contour[i].xyz[j].x;
                      tmp_y[1] = misc->hd_contour[i].xyz[j].y;
                      ...
                      if (tmp_x[0] >= misc->total_displayed_area.min_x && tmp_x[0] <= misc->total_displayed_area.max_x &&
                          ...
                          tmp_y[1] >= misc->total_displayed_area.min_y && tmp_y[1] <= misc->total_displayed_area.max_y)
                        {
                          map->drawLine (tmp_x[0], tmp_y[0], tmp_x[1], tmp_y[1], options->contour_highlight_color, FAT_LINE_WIDTH, NVFalse, Qt::SolidLine);
                        }
