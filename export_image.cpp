...
void export_image (QWidget *parent, MISC *misc, OPTIONS *options, nvMap *map)
{
  ...
  QFileDialog *fd = new QFileDialog (parent, gridEdit::tr ("gridEdit Export Image File"));
  ...
  if (fd->exec () == QDialog::Accepted)
      ...
      if (!file.isEmpty())
	{
	  ...
	  if (file.endsWith (".jpg") || file.endsWith (".jpeg") || file.endsWith (".JPG") || file.endsWith (".JPEG")) type = 1;
	  ...
	  switch (type)
		...
		{
		  QMessageBox::warning (parent, gridEdit::tr ("gridEdit Export GeoTIFF"), gridEdit::tr ("Environment variable GDAL_DATA is not set!"));
		  return;
		}
	      ...
	      if (!gt)
		{
		  QMessageBox::warning (parent, gridEdit::tr ("gridEdit Export GeoTIFF"),
					gridEdit::tr ("Could not get GTiff driver."));
		  return;
		}
		...
		{
		  QMessageBox::warning (parent, gridEdit::tr ("gridEdit Export GeoTIFF"),
					gridEdit::tr ("Could not create GeoTIFF file ") + file);
		  return;
		}
	      ...
	      uint8_t *alpha = (uint8_t *) calloc (width, sizeof (uint8_t));
              ...
              misc->statusProgLabel->setText (gridEdit::tr (" Exporting GeoTIFF "));
	      ...
	      for (int32_t i = 0 ; i < height ; i++)
		{
		  ...
		  if (err == CE_Failure)
		    {
		      QMessageBox::warning (parent, gridEdit::tr ("gridEdit Export GeoTIFF"),
					    gridEdit::tr ("Failed TIFF scanline write."));
